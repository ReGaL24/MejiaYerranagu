...
The formal analysis is split into two Alloy models to address two different correctness concerns of BBP. Separating the models keeps each one focused and avoids mixing trip-recording behavior with business rules governing path contribution and publication.

\textbf{BestBikePath Trip Recording:}

The BestBikePath (BBP) Trip Recording system is designed to record and manage bike trips for users. The system captures essential trip details, including start and end locations, timestamps, and user information. The goal for this first model is to ensure accurate trip recording while maintaining data integrity and user privacy.
...
The following figure shows the result of running the above Alloy model in the Alloy Analyzer. It shows that the assertions we defined hold true for the given scope.
