...
The data layer consists of a cluster of relational database management system (PostgreSQL) for persistent storage of user data, trips, paths, and other relevant information. 
...
\begin{center}
    \includegraphics[width=0.8\textwidth]{Images/System view.png}
\end{center}
...
\end{itemize}
These microservices are independently deployable and scalable, allowing for rapid development and updates without affecting the entire system. They communicate with each other using RESTful APIs for event-driven interactions through message boards like RabbitMQ for events such as $trip.published, path.published,$ 
$ weather.enriched, weather.failed$.
...
\includegraphics[scale=0.43]{Images/Runtime_UC4.png}
...
\begin{itemize}
    ...
    \item ``Path'' represents a bike path published by users, with geometry (LineString - courtesy of PostGIS) and status information.
    \item ``PathScore'' is a computed metric (0-10) based on path status quality and routing effectiveness.
...
\textbf{Cache Server - Redis Interface}
...
\begin{itemize}
\item[-] cache\_tripData(TripID, TripData, TTL=48h) -- Cache trip data during offline recording
\item[-] cache\_weatherData(Location, WeatherData, TTL=24h) -- Cache weather results
\item[-] cache\_pathQueryResult(Query, Results, TTL=2h) -- Cache Origin-Destination path search results
\end{itemize}

\textbf{Retrieval Operations:}

\begin{itemize}
\item[-] get\_cachedGeocoding(Address) -- Retrieve cached coordinates
\item[-] get\_cachedWeather(Location) -- Retrieve cached weather
\item[-] get\_cachedQueryResult(Query) -- Retrieve cached search results
\end{itemize}

\textbf{Invalidation Operations:}
...
\textbf{PathScore Computation Logic}
...
\begin{table}[h!]
    \centering
    \caption{Path Score Weights}
    \label{path-score-weights}
    \begin{tabular}{|c|c|c|}
        \hline
    ...
    \caption{Obstacle Weights}
    \label{obstacle-weights}
    \begin{tabular}{|c|c|c|}
        \hline
...
The path score is computed as follows: 
Step 1: Calculating obstacle score: 
{\centering
...
\textbf{Automated Anomaly Detection}
...
\end{center}
Where $\omega_x$, $\omega_y$, and $\omega_z$ are the gyroscope readings along the x, y, and z axes respectively. When a sudden change in angular velocity is detected, an obstacle event is triggered.
A combination of both accelerometer and gyroscope data can be used to improve the accuracy of obstacle detection. For example, an obstacle event is only triggered when both the energy from the accelerometer and the angular velocity from the gyroscope exceed their respective thresholds. This is used to negate false positives from sudden movements such as breaking hard, other road undulations that are not related to obstacles.
Further the can account for speed of the cyclist by using the GPS data to adjust sensitivity of the obstacle detection algorithm. At higher speeds, the thresholds for energy and angular velocity can be increased to reduce false positives, while at lower speeds, the thresholds can be decreased to improve sensitivity.
