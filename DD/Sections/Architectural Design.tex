...
This section aims to provide a high-level description of the architecture of the BestBikePaths (BBP) system. It also outlines the main architectural layers, the core components and their interactions. This section also validates the modeling decisions made to satisfy both the functional and non-functional requirements of the system.
...
\begin{itemize}
    ...
    \item Path Query Service: Handles origin-destination queries for bike paths for both \\ \verb|RegisteredUser| and \verb|UnregisteredUser|, ranking them based on path score and status.
...
\begin{itemize}
    ...
    \item Authentication and User Management Service: This component oversees managing user accounts, credentials and sessions. It validates authentication tokens and controls access to restricted functionalities, specifically when regarding the distinction between registered and unregistered users. Thus, it supports both authenticated and guest sessions.
    \item Trip Management Service: This component handles the storage and retrieval of previously recorded trips. It can compute and persist relevant trip statistics to deliver them to the User Interface. It also manages trip lifecycle states like recorded, processed, enriched, and communicates with Weather Enrichment Service to retrieve and store additional trip information. Since this service displays trips for the user, it retrieves coordinates data from the Geocoding Integration Service.
    ...
    \item Weather Enrichment Service: This component communicates directly with the external weather service, described in the External Services section below. It communicates with the Trip Management Service to complement the stored trip information. It isolates the dependency with the third-party weather service from the core application logic.
    \item Notifications Service: This component manages system notifications regarding trip completion, errors or review requests, to enrich the user’s experience with relevant information. This component handles exclusively in-app and push notifications.
\end{itemize}
...
\begin{itemize}
    \item Weather Service: This external component retrieves meteorological data given specific location and time data. This service is exposed as an API that can be consumed by the Weather Enrichment Service on demand.
...
\begin{center}
    \includegraphics[]{Images/ComponentView.png}
\end{center}
...
The Microservice Application Servers host the core backend services of the BBP system. Each microservice is deployed on its own server instance, allowing for independent scaling and maintenance. The microservices communicate with each other using RESTful APIs and also interact with the database for data storage and retrieval. The microservices include:
 \begin{itemize}
    ...
    \item Path Management Service - for handling of manual path creation by \verb|RegisteredUser|, validation and managing discard/draft/publish states, and storage of path data. in the database
    \item Path Query Service - for handling of path search and retrieval for Origin-Destination queries, computing of path scores, and ranking paths based on it.
...
\begin{itemize}
    ...
    % \item[-] put\_updatePathStatus(UserID, PathID, NewStatus)
\end{itemize}
...
% This service manages manual path entry and its creation workflow.
...
% \begin{itemize}
%     \item[-] put\_updatePathStatus(UserID, PathID, NewStatus) -- Change path status (Optimal/Medium/Sufficient/Maintenance)
% \end{itemize}
...
\textbf{Path Query Service Interface}

% Executes origin-destination queries and path ranking.
...
\begin{itemize}
    \item[-] get\_pathScore(PathID) -- Get current PathScore calculation
...
\begin{itemize}
    ...
    \item[-] post\_enrichTrip(TripID, WeatherData) -- Store weather with trip
\end{itemize}

\textbf{Notification Service Interface}

% Listens to events, generates notifications, sends push and email.
